...
void loop() 
{
 if (irrecv.decode(&results))
   ...
   { // 接收紅外線訊號並解碼
    if(results.value==0xFF629D) //輸入自己的紅外線編碼
    {
  forward(Rspeed,Lspeed); //前進
    }
if(results.value==0xFFA857) //輸入自己的紅外線編碼
    {
   back(Rspeed,Lspeed); //後退
   }
if(results.value==0xFF22DD) //輸入自己的紅外線編碼
    {
  left(Rspeed,Lspeed); //左轉
    }
if(results.value==0xFFC23D) //輸入自己的紅外線編碼
    {
  right(Rspeed,Lspeed); //右轉
   }
if(results.value==0xFF02FD) //輸入自己的紅外線編碼
    {
  pause(0,0); //停止
    }
